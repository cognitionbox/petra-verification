...
@Primative
public interface StrategiesEffectedView {
    PCollection<Strategy> strategies();
    default boolean strategiesEffected() {
        return strategies().forall(s ->
                s.strategyState().isOpenBuyAndShouldWait() ^ s.strategyState().isOpenBuyAndShouldStop() ^ s.strategyState().isOpenBuyAndShouldExit() ^
                        s.strategyState().isOpenSellAndShouldWait() ^ s.strategyState().isOpenSellAndShouldStop() ^ s.strategyState().isOpenSellAndShouldExit() ^
                        s.strategyState().isBuy() ^ s.strategyState().isSell());
    }
}
